...
#End Region
    ...
    Private Sub BarButtonItem6_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem6.ItemClick
        Try
            ...
            Ten_mon = cmbID_mon.SelectedItem("Ten_mon_hoc").ToString & " ( " & cmbID_mon.SelectedItem("So_hoc_trinh").ToString & " tín chỉ )"
            Hoc_ky = cmbHoc_ky.Text
            Nam_hoc = cmbNam_hoc.Text
            Dim para(2) As SqlClient.SqlParameter
            para(0) = New SqlClient.SqlParameter("@ID_lop", ID_Lop)
            ...
            Dim report As New rptBangTongHopDiemKT(dtHeSo1.DefaultView, dvHeSo2, dv, Ten_mon)
    ...
    Private Sub btnBaoCaoHoanThanhMonHoc_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBaoCaoHoanThanhMonHoc.ItemClick
        Try
            Dim obj As New ChuongTrinhDaoTaoChiTiet
            Dim Ten_lop As String = ""
            Dim Ten_mon As String = ""
            Dim So_tin_chi As String = ""
            Dim Ten_khoa As String = ""
            Dim dv As DataView = grdViewDiem.DataSource
            If dv Is Nothing Then
                Thongbao("Chưa đầy đủ thông tin. Hãy chắc rằng bạn đã chọn đúng Lớp hoặc Tên học phần")
            Else
                Ten_mon = cmbID_mon.SelectedItem("Ten_mon_hoc").ToString
                So_tin_chi = cmbID_mon.SelectedItem("So_hoc_trinh").ToString
                Ten_lop = dv.Item(0)("Ten_lop").ToString
                Ten_khoa = dv.Item(0)("Ten_khoa").ToString
                Dim rpt As New rpt_BaoCaoHoanThanhGiangDayMonHoc(Ten_lop, Ten_mon, So_tin_chi, Ten_khoa)
                PrintXtraReport(rpt)
            End If
        Catch ex As Exception
            Thongbao(ex.Message)
    ...
    Private Sub btnBangDiemThanhPhan_14_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBangDiemThanhPhan_14.ItemClick
        Try
            Dim dv As DataView = grdViewDiem.DataSource
            Dim dtTP As DataTable
            dtTP = clsDiem.DanhSachThanhPhanChon
            Dim report As New rpt_TongHopDiemKT_DinhKy_Mau02(dtTP, dv, cmbID_mon.Text)
            Dim printTool As New ReportPrintTool(report)
            printTool.ShowPreviewDialog()
        Catch ex As Exception
            Thongbao(ex.Message)
    ...
    Private Sub BarButtonItem7_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem7.ItemClick
        Try
            Dim rpt As New rpt_BaoCaoKetThucKiemTraMonHoc
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
