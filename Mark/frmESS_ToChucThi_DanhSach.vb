...
#End Region
    ...
    Private Sub btnPhieuTheoDoiKetQua_DanPT_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnPhieuTheoDoiKetQua_DanPT.ItemClick
        Try
            ...
            If dv.Count > 0 Then
                ...
                dt_distintsc = grcViewDanhSachThi.DataSource.ToTable(True, "Ten_lop")
    ...
    Private Sub BarButtonItem6_ItemClick_1(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem6.ItemClick
        Try
            ...
            Dim ID_phong_thi As Integer = trvPhongThi.ID_phong_thi
            Dim dtPhongThi As DataTable = ESS.Machine.UDB.SelectTable("Select * FROM MARK_TochucThiPhong WHERE ID_phong_thi=" & ID_phong_thi)
            Dim Ten_phong As String = dtPhongThi.Rows(0)("Ten_phong")
            Dim Tieu_de_bao_cao As String = " DANH SÁCH THÍ SINH THAM GIA THI "
            Dim rpt As New rpt_DanhSachDanPhongThi_New(dt, Ten_phong, Tieu_de_bao_cao)
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    ...
    Private Sub BarButtonItem7_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem7.ItemClick
        Try
            ...
            Dim dtPhongThi As DataTable = ESS.Machine.UDB.SelectTable("Select * FROM MARK_TochucThiPhong WHERE ID_phong_thi=" & ID_phong_thi)
            Dim Ten_phong As String = dtPhongThi.Rows(0)("Ten_phong")
            Dim Tieu_de_bao_cao As String = " DANH SÁCH THÍ SINH THAM GIA THI TỐT NGHIỆP "
            Dim rpt As New rpt_DanhSachDanPhongThi_New(dt, Ten_phong, Tieu_de_bao_cao)
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
...
#Region "Biên bản thông tư 09"
    Private Sub btnBienBanBanGiaoBaiThi_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBienBanBanGiaoBaiThi.ItemClick
        Try
            Dim rpt As New rpt_BienBanBanGiaoBaiThi
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    End Sub
    Private Sub btnBienBanHSSVBoThiTN_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBienBanHSSVBoThiTN.ItemClick
        Try
            Dim rpt As New rpt_BienBanHSSVBoThiTN
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    End Sub
    Private Sub btnBienBanViPhamNoiQuyThi_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBienBanViPhamNoiQuyThi.ItemClick
        Try
            Dim rpt As New rpt_BienBanViPhamNoiQuyPhongThi
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    End Sub
    Private Sub btnBienBanGiamSatThiTN_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBienBanGiamSatThiTN.ItemClick
        Try
            Dim rpt As New rpt_BienBanGiamSatThiTN
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    End Sub
    Private Sub btnBienBanGiamSatThi_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnBienBanGiamSatThi.ItemClick
        Try
            Dim Ten_mon As String = trvPhongThi.Ten_mon
            Dim Ten_lop As String = ","
            Dim dt_distintsc As New DataTable
            dt_distintsc = grdViewDanhSachThi.DataSource.ToTable(True, "Ten_lop")
            For i As Integer = 0 To dt_distintsc.DefaultView.Count - 1
                Ten_lop = Ten_lop & dt_distintsc.Rows(i)("Ten_lop").ToString & ","
            Next
            Ten_lop = Ten_lop.ToString.Substring(1, Ten_lop.Length - 2)
            Dim ID_thi As Integer = trvPhongThi.ID_thi
            Dim dt_marktochucthi As New DataTable
            dt_marktochucthi = ESS.Machine.UDB.SelectTable("SELECT tc.*,FORMAT(Ngay_thi,'dd-MM-yyyy') as Ngay_thi_fomart FROM MARK_TochucThi as tc WHERE ID_thi =" & ID_thi)
            Dim Ngay_thi As String = ""
            If dt_marktochucthi.Rows.Count > 0 Then
                Ngay_thi = dt_marktochucthi.Rows(0)("Ngay_thi").ToShortDateString
            End If
            Dim rpt As New rpt_BienBanGiamSat(Ten_mon, Ten_lop, Ngay_thi)
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
    End Sub
    Private Sub btnPhieuChamKiemTraVanDap_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnPhieuChamKiemTraVanDap.ItemClick
        Try
            Dim rpt As New rpt_PhieuChamKiemTraVanDap
            PrintXtraReport(rpt)
        Catch ex As Exception
            Thongbao(ex.Message)
        End Try
