    ...
    Private Sub TrvLop_ChuanHoa_TreeNodeSelected_() Handles TrvLop_ChuanHoa.TreeNodeSelected_
        Try
            If Not TrvLop_ChuanHoa.ID_lop_list Is Nothing Then
                ...
                Dim SQL As String = "SELECT a.ID_SV FROM Mark_DanhSachLuanVan a " & _
                                "INNER JOIN STU_DanhSach b ON a.ID_sv=b.ID_sv " & _
                                "WHERE ID_lop IN (" & TrvLop_ChuanHoa.ID_lop_list & ") AND Lan_xet <>" & cmbLan_xet.Text & " " & _
                                "UNION SELECT a.ID_SV FROM Mark_DanhSachThiTotNghiep a " & _
                                "INNER JOIN STU_DanhSach b ON a.ID_sv=b.ID_sv " & _
                                "WHERE ID_lop IN (" & TrvLop_ChuanHoa.ID_lop_list & ") AND Lan_xet <>" & cmbLan_xet.Text & _
                                "UNION SELECT ID_SV FROM STU_DanhSach WHERE ID_lop IN (" & TrvLop_ChuanHoa.ID_lop_list & ")  AND Trang_thai<>0"
                'Load danh sách sinh viên
                Dim objDanhSach As New DanhSachSinhVien_BLL(TrvLop_ChuanHoa.ID_lop_list, SQL)
...
#End Region
    ...
    Private Sub btnToTucThi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnxet.Click
        Try
            ...
            ElseIf IsNumeric(txtTBCHT.Text.Trim) Then
                ...
                Dim SQL As String = "DELETE MARK_DanhSachLuanVan WHERE ID_SV in (SELECT ID_SV FROM STU_DANHSACH WHERE ID_lop in (" & TrvLop_ChuanHoa.ID_lop_list & ")) AND Lan_xet=" & cmbLan_xet.Text.Trim
                Dim clsDM As New DanhMuc_BLL
                clsDM.Execute(SQL)
                SQL = "DELETE MARK_DanhSachThiTotNghiep WHERE ID_SV in (SELECT ID_SV FROM STU_DANHSACH WHERE ID_lop in (" & TrvLop_ChuanHoa.ID_lop_list & ")) AND Lan_xet=" & cmbLan_xet.Text.Trim
                clsDM.Execute(SQL)
                SQL = "DELETE MARK_DanhSachNoTotNghiep WHERE ID_SV in (SELECT ID_SV FROM STU_DANHSACH WHERE ID_lop in (" & TrvLop_ChuanHoa.ID_lop_list & ")) "
                clsDM.Execute(SQL)
            ...
            End If
            Thongbao("Xét thành công !", MsgBoxStyle.Information, "Thông báo !")
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            Select Case TabControl1.SelectedIndex.ToString
                ...
                '    End If
                '    Tieu_de1 = "DANH SÁCH SINH VIÊN LÀM ĐỒ ÁN/ KHÓA LUẬN XÉT LẦN " & cmbLan_xet.Text
                ...
                Case 0
                    ...
                    Else
                        Thongbao("Chưa có dữ liệu !")
                    End If
                    Tieu_de1 = "DANH SÁCH ĐỦ ĐIỀU KIỆN DỰ THI TỐT NGHIỆP "
                ...
                Case 1
                    ...
                    'Select Case Id_chuyen_nganh
                    '    Case 1, 2, 3, 4, 11, 12, 13, 14, 15
                    '        Tieu_de1 = "DANH SÁCH SINH VIÊN KHÔNG ĐỦ ĐIỀU KIỆN BẢO VỆ ĐỒ ÁN/ KHÓA LUẬN XÉT LẦN " & cmbLan_xet.Text
                    '    Case Else
                    '        Tieu_de1 = "DANH SÁCH SINH VIÊN KHÔNG ĐỦ ĐIỀU KIỆN THI TỐT NGHIỆP XÉT LẦN " & cmbLan_xet.Text
                    'End Select
                    Tieu_de1 = "DANH SÁCH KHÔNG ĐỦ ĐIỀU KIỆN DỰ THI TỐT NGHIỆP "
            ...
            If Thi < 2 Then
                ...
                PrintXtraReport(rpt)
            Else
                Dim rpt As New rptMark_DanhSachSinhVien_NoThiTotNghiep(dv, Tieu_de1, Tieu_de2)
                PrintXtraReport(rpt)
            End If
        Catch ex As Exception
        End Try
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Try
            ...
            Dim dv As DataView = grdViewNoThiTotNghiep.DataSource
            If dv.Count > 0 Then
                clsXet.ChuyenThiTotNghiep_No(dv)
                'Load danh sach sinh thuoc dien da xet lam luan van
                clsXet = New DanhSachLuanVanThiNoTotNghiep_BLL(mID_he, gobjUser.ID_dv, gobjUser.ID_Bomon, TrvLop_ChuanHoa.ID_lop_list, 0, dt_ds_lv)
                Dim dt As DataTable = clsXet.Load_DanhSach_LuanVan(cmbLan_xet.Text)
                grcDanhSach.DataSource = dt.DefaultView
    ...
    Private Sub cmbLan_xet_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbLan_xet.SelectedIndexChanged
        TrvLop_ChuanHoa_TreeNodeSelected_
    End Sub
End Class